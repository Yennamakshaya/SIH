    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-900 to-slate-900 text-white rounded-2xl p-6 shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold mt-1">
            {t('welcomeBuyer')} {summary?.company_name || "Shree Foods Pvt Ltd"} 🏢
          </h1>
          <p className="text-xs text-blue-200 mt-1">
            {t('contactPerson')}: {summary?.contact_person || "Srinivas Rao"} | {t('status')}: <span className="text-emerald-400 font-bold">{t('statusVerifiedBuyer')}</span> | {t('reliability')}: {summary?.reliability_score || 94}% ⭐ {summary?.rating || 4.7}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          ...
          <span className="text-[11px] text-blue-600 font-medium">1 {t('activeProcurement')}</span>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 space-y-4">
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
          <div>
            <span className="text-slate-500">{t('cropRequired')}</span>
            <p className="font-extrabold text-slate-900 text-sm">Tomato (Grade A)</p>
          </div>
          <div>
            <span className="text-slate-500">{t('quantityNeeded')}</span>
            <p className="font-bold text-slate-800 text-sm">5,000 kg</p>
          </div>
          <div>
            <span className="text-slate-500">{t('maxBudgetPrice')}</span>
            <p className="font-extrabold text-emerald-700 text-sm">₹32/kg</p>
          </div>
          <div>
            <span className="text-slate-500">{t('preferredRegion')}</span>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-base text-slate-800 flex items-center gap-2">
              ...
              {t('smartRecommendedFarmers')}
            </h3>
            <p className="text-xs text-slate-500">{t('rankedByMatch')}</p>
          </div>
        </div>
